...
import { useHistoricalData, useSortState, useInvertedPairs } from '../../hooks';
import {
  calculateStatus,
  getAdjustedPosition,
  sortPositions,
...
export const PositionsList = memo(
  ({ positions, showWallet = false, historyEnabled = false }) => {
    // Custom hooks for state management
    const { sortState, handleSort } = useSortState('age', 'desc');
    const { invertedPairs, handlePairInversion, isInverted } =
      useInvertedPairs();
    ...
    const handleShowCard = useCallback(
      (position) => {
        ...
        const adjustedPosition = {
          ...
          pairDisplay: isPairInverted
            ? invertPairString(position.pair)
            : position.pair,
        };
